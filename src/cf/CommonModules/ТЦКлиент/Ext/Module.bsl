...
// Обрабатывает ПараметрЗапуска и, если нужно, выполняет запуск подсистемы Тест-центра.
// 
Процедура ПриНачалеРаботыСистемы() Экспорт
	...
	Исключение
		СообщениеОбОшибке = "Произошла ошибка при обработке параметров запуска Тест-центра:"
							+ Символы.ПС + ТЦОбщий.ИнформациюОбОшибкеВСтроку(ИнформацияОбОшибке());
		ТЦОбщий.ЗаписатьВЖурнал(СообщениеОбОшибке, "Ошибка при запуске Тест-центра", ПредопределенноеЗначение("Перечисление.ТЦСтатусСообщения.Ошибка"));
...
Функция ОбработатьОжиданиеВРМ(Контекст, Форма) Экспорт
	...
	Попытка
		...
		Если Форма.ВозниклоИсключение Или Форма.РезультатВыполнения <> Продолжение Тогда
			...
			ЗаписатьРезультатыОценкиПроизводительности();
			Если Форма.ВозниклоИсключение ИЛИ Форма.РезультатВыполнения = Контекст.Перечисления.ТЦРезультатВыполнения.Ошибка Тогда
				
				ОтветноеСообщение = Ошибка;
				Если ЗначениеЗаполнено(Форма.ТекстОшибкиОбработки) Тогда
				...
				Иначе
					ПараметрыОтвета.Результат = "Тестовая обработка вернула статус ""выполнено с ошибкой""";							
				КонецЕсли;
...
Функция ВыполнитьОпросУправленияТестомДляРолей() Экспорт
	...
	Если ТЦТипПриложения = ПредопределенноеЗначение("Перечисление.ТЦТипКлиентскогоПриложения.УправлениеТестом") Тогда
		//логика работы клиента для типа УправлениеТестом
		ПроверкаДляТипаУправлениеТестом(Результат);
		
	ИначеЕсли ТЦТипПриложения = ПредопределенноеЗначение("Перечисление.ТЦТипКлиентскогоПриложения.АгентТЦ") Тогда
		ПроверкаДляТипаАгентТЦ(Результат);
		
	ИначеЕсли Не ЗначениеЗаполнено(ТЦТипПриложения) Тогда
		//Если роль не назначена, то переходим в режим ожидания
		ПереключитьРежимОпроса(Ложь);
	КонецЕсли;
	
	Возврат ТЦТипПриложения;
