...
Процедура ДобавитьСтруктуруИтерацииВТест(Тест, НомерИтерации, ФактическаяСтруктура)
	...
	СтруктураИтераций = ТестОбъект.СтруктураИтераций;
	КоличествоСтрокИтераций = СтруктураИтераций.Количество();
	ТекущийНомерСтроки = КоличествоСтрокИтераций - 1;
	Пока ТекущийНомерСтроки >= 0 Цикл
		
		Если СтруктураИтераций[ТекущийНомерСтроки].НомерИтерации <> НомерИтерации Тогда
			Прервать;
		КонецЕсли;
		
		СтруктураИтераций.Удалить(ТекущийНомерСтроки);		
		ТекущийНомерСтроки = ТекущийНомерСтроки - 1;
		
	КонецЦикла;
	
	Для Каждого СтрокаИтерации Из ФактическаяСтруктура Цикл
		НоваяСтрока = СтруктураИтераций.Добавить();
...
Функция ПолучитьМасштабированнуюСтруктуруСценария(Сценарий, СценарийПредыдущейИтерации,
	...
	Если ИсходноеКоличествоПользователей <> ЦелевоеКоличествоПользователей Тогда
		...
		// Масштабируем сценарий по ролям
		ВыборкаРолей = Результат.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкамСИерархией);
		Пока ВыборкаРолей.Следующий() И ЦелевоеКоличествоПользователейОсталось > 0 Цикл
			
			ТекущееКоличествоПоРоли = Окр(ВыборкаРолей.Количество * ЦелевоеКоличествоПользователейОсталось / ИсходноеКоличествоПользователейОсталось);
			ЦелевоеКоличествоПользователейОсталось	= ЦелевоеКоличествоПользователейОсталось - ТекущееКоличествоПоРоли;
			ИсходноеКоличествоПользователейОсталось	= ИсходноеКоличествоПользователейОсталось - ВыборкаРолей.Количество;
			
			Если ТекущееКоличествоПоРоли = 0 Тогда
				Продолжить;
			КонецЕсли;
				
			ИсходноеКоличествоПоРолиОсталось = ВыборкаРолей.Количество;
			
			// Масштабируем по клиентам
			ВыборкаКлиентов = ВыборкаРолей.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкамСИерархией);
			Пока ВыборкаКлиентов.Следующий() Цикл
				
				ТекущееКоличествоПоКлиенту = Окр(ВыборкаКлиентов.Количество * ТекущееКоличествоПоРоли / ИсходноеКоличествоПоРолиОсталось);
				ТекущееКоличествоПоРоли = ТекущееКоличествоПоРоли - ТекущееКоличествоПоКлиенту;
				ИсходноеКоличествоПоРолиОсталось = ИсходноеКоличествоПоРолиОсталось - ВыборкаКлиентов.Количество;
				
				Если ТекущееКоличествоПоКлиенту = 0 Тогда
					Продолжить;
				КонецЕсли;
				
				ИсходноеКоличествоПоКлиентуОсталось = ВыборкаКлиентов.Количество;
				
				// Масштабируем по пользователям
				ВыборкаПоПользователям = ВыборкаКлиентов.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкамСИерархией);
				Пока ВыборкаПоПользователям.Следующий() Цикл
					
					ТекущееКоличествоПоПользователю = Окр(ВыборкаПоПользователям.Количество * ТекущееКоличествоПоКлиенту / ИсходноеКоличествоПоКлиентуОсталось);
					ТекущееКоличествоПоКлиенту = ТекущееКоличествоПоКлиенту - ТекущееКоличествоПоПользователю;
					ИсходноеКоличествоПоКлиентуОсталось = ИсходноеКоличествоПоКлиентуОсталось - ВыборкаПоПользователям.Количество;
					
					НоваяСтрока = СтруктураСценария.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, ВыборкаПоПользователям);
					НоваяСтрока.Количество = ТекущееКоличествоПоПользователю;
	...
	Иначе
		СтруктураСценария = Сценарий.Структура.Выгрузить(,"Роль,Клиент,Пользователь,Количество");
		СтруктураСценария.Свернуть("Роль,Клиент,Пользователь", "Количество");
	КонецЕсли;
	
	СтруктураСценария.Сортировать("Количество,Роль,Клиент,Пользователь");
...
// Отмечает указанное количество ВРМ как готовые к запуску или загрузке
//
// Параметры:
//  ОбщееКоличествоОдновременноЗапускаемыхВРМ  - Число
//	СостояниеГотовКДействию		— Перечисления.ТЦСостояниеПодготовкиВРМ.ГотовКЗапуску или Перечисления.ТЦСостояниеПодготовкиВРМ.ГотовКЗагрузке
//	СостояниеНеготовКДействию	— Перечисления.ТЦСостояниеПодготовкиВРМ.ГотовКЗагрузке или Перечисления.ТЦСостояниеПодготовкиВРМ.НеГотовКЗагрузке
//
Процедура ОтметитьВРМКЗапускуИлиЗагрузке(ОбщееКоличествоОдновременноЗапускаемыхВРМ, СостояниеНеготовКДействию, СостояниеГотовКДействию)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               |	ЕСТЬNULL(СУММА(ВЫБОР
	               |				КОГДА ТЦСценарийЗапуска.СостояниеПодготовки = &СостояниеГотовКДействию
	               |					ТОГДА 1
	               |			КОНЕЦ), 0) КАК КоличествоГотовых,
	               |	ЕСТЬNULL(СУММА(ВЫБОР
	               |				КОГДА ТЦСценарийЗапуска.СостояниеПодготовки = &СостояниеНеготовКДействию
	               |					ТОГДА 1
	               |			КОНЕЦ), 0) КАК КоличествоНеготовых
	               |ИЗ
	               |	РегистрСведений.ТЦСценарийЗапуска КАК ТЦСценарийЗапуска";
	
	Запрос.УстановитьПараметр("СостояниеГотовКДействию", СостояниеГотовКДействию);
	Запрос.УстановитьПараметр("СостояниеНеготовКДействию", СостояниеНеготовКДействию);
	...
	Если ВРМКОтметке < 1 Тогда
		Возврат;
	КонецЕсли;

	НачатьТранзакцию();
	Запрос.Текст = "ВЫБРАТЬ
	               |	ТЦСценарийЗапуска.Агент КАК Агент,
	               |	КОЛИЧЕСТВО(*) КАК ВсегоНеготовыхПоАгенту
	               |ИЗ
	               |	РегистрСведений.ТЦСценарийЗапуска КАК ТЦСценарийЗапуска
	               |ГДЕ
	               |	ТЦСценарийЗапуска.СостояниеПодготовки = &СостояниеНеготовКДействию
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ТЦСценарийЗапуска.Агент
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	ВсегоНеготовыхПоАгенту Возр";
	
	Результат  = Запрос.Выполнить();
	ЗафиксироватьТранзакцию();
	
	КОтметкеПоАгентам = Результат.Выгрузить(); 
	КОтметкеПоАгентам.Колонки.Добавить("КОтметке", Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(10, 0, ДопустимыйЗнак.Неотрицательный)));
	
	ОсталосьАгентов = КОтметкеПоАгентам.Количество();
	Для Каждого ТекСтрока Из КОтметкеПоАгентам Цикл

		Если ВРМКОтметке < 1 Тогда
			Прервать;
		КонецЕсли;
		
		ТекСтрока.КОтметке = Мин(Окр(ВРМКОтметке / ОсталосьАгентов), ТекСтрока.ВсегоНеготовыхПоАгенту);
		ОсталосьАгентов = ОсталосьАгентов - 1;
		ВРМКОтметке = ВРМКОтметке - ТекСтрока.КОтметке;
		
	КонецЦикла;

	Если КОтметкеПоАгентам.Итог("КОтметке") < 1 Тогда
		Возврат;
	КонецЕсли;
	...
	ШаблонЗапроса = "
                    ...
                    |	РегистрСведений.ТЦСценарийЗапуска КАК ТЦСценарийЗапуска
                    |ГДЕ
                    |	ТЦСценарийЗапуска.Агент = &%2
					|	И ТЦСценарийЗапуска.СостояниеПодготовки = &СостояниеНеготовКДействию
					|";
	
	ЗапросПоАгентам.УстановитьПараметр("СостояниеНеготовКДействию", СостояниеНеготовКДействию);

	Сч = 1;
	МассивТекстов = Новый Массив;
	Для Каждого ТекСтрока Из КОтметкеПоАгентам Цикл
		
		Если ТекСтрока.КОтметке < 1 Тогда
			Продолжить;
		КонецЕсли;
		
		ТекПараметрАгент = "Агент" + Формат(Сч, "ЧГ=0");
		
		МассивТекстов.Добавить(
			СтрШаблон(ШаблонЗапроса, Формат(ТекСтрока.КОтметке, "ЧГ=0"), ТекПараметрАгент)
		);
		
		ЗапросПоАгентам.УстановитьПараметр(ТекПараметрАгент, ТекСтрока.Агент);
		Сч = Сч + 1;

	КонецЦикла;

	ЗапросПоАгентам.Текст = СтрСоединить(МассивТекстов, "ОБЪЕДИНИТЬ ВСЕ");
	Выборка = ЗапросПоАгентам.Выполнить().Выбрать();

	Пока Выборка.Следующий() Цикл
		ОтметитьСостояниеПодготовкиВРМ(Выборка.Ид, СостояниеГотовКДействию);
	КонецЦикла;
...
Процедура ОтметитьВРМКЗапуску(ОбщееКоличествоОдновременноЗапускаемыхВРМ)
	...
	ОтметитьВРМКЗапускуИлиЗагрузке(ОбщееКоличествоОдновременноЗапускаемыхВРМ,
									Перечисления.ТЦСостояниеПодготовкиВРМ.НеГотовКЗапуску,
									Перечисления.ТЦСостояниеПодготовкиВРМ.ГотовКЗапуску);
...
Процедура ОтметитьВРМКЗагрузке(ОбщееКоличествоОдновременноЗапускаемыхВРМ) 
	...
	ОтметитьВРМКЗапускуИлиЗагрузке(ОбщееКоличествоОдновременноЗапускаемыхВРМ,
									Перечисления.ТЦСостояниеПодготовкиВРМ.НеГотовКЗагрузке,
									Перечисления.ТЦСостояниеПодготовкиВРМ.ГотовКЗагрузке);
...
Процедура ПодготовитьСценарийЗапуска(СтруктураСценария, ИспользоватьТолькоКлонов, ПредыдущийСценарийЗапуска, ЭтоДинамическийТест = Ложь, ОбщееКоличествоОдновременноЗапускаемыхВРМ = 0) Экспорт
	...
	СтруктураСценария.Сортировать("Компьютер,Роль,Клиент,Пользователь");
...
Функция ПереиспользоватьВРМ(Компьютер, НомерСессии, ПродолжающиеРаботуВРМ, ОбщееКоличествоОдновременноЗапускаемыхВРМ) Экспорт
	...
	РезультатЗапросаСценарияЗапуска = Запрос.Выполнить();
	ВыборкаСценарияЗапуска = РезультатЗапросаСценарияЗапуска.Выбрать();
	...
	Попытка
		...
		Если Метаданные.РежимУправленияБлокировкойДанных = Метаданные.СвойстваОбъектов.РежимУправленияБлокировкойДанныхПоУмолчанию.Управляемый Тогда
			...
			ЭлементБлокировки.ИсточникДанных = РезультатЗапросаСценарияЗапуска;
			ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Ссылка", "ВРМ");
			Блокировка.Заблокировать();
		КонецЕсли;
		
		Пока ВыборкаСценарияЗапуска.Следующий() Цикл
			
			
			НайденВПродолжающихРаботу = Истина;
			Если ПродолжающиеРаботуВРМ <> Неопределено
				И ПродолжающиеРаботуВРМ.Найти(ВыборкаСценарияЗапуска.ВРМ) = Неопределено Тогда
				
				НайденВПродолжающихРаботу = Ложь;
				
				ОбъектВП = ВыборкаСценарияЗапуска.ВРМ.ПолучитьОбъект();
				ОбъектВП.Роль = ВыборкаСценарияЗапуска.РольЭлементаСценария;
			...
			СтрокиСценарияЗапуска.Отбор.Ид.Установить(ВыборкаСценарияЗапуска.ИдЭлементаСценария);
			...
			НомераВРМ[ВыборкаСценарияЗапуска.ВРМ] = ВыборкаСценарияЗапуска.Номер;
		КонецЦикла;
...
Функция НайтиКомпьютер(ИмяКомпьютера) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ГДЕ
	               |	ТЦКомпьютеры.Наименование = &ИмяКомпьютера";
	Выборка = Запрос.Выполнить().Выбрать();
	...
	Если Метаданные.РежимУправленияБлокировкойДанных = Метаданные.СвойстваОбъектов.РежимУправленияБлокировкойДанныхПоУмолчанию.Управляемый Тогда
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("Справочник.ТЦКомпьютеры");
		ЭлементБлокировки.УстановитьЗначение("Наименование", ИмяКомпьютера);
		Блокировка.Заблокировать();
	КонецЕсли;

	// Другой участник успел зарегистрировать компьютер, пока устанавливалась блокировка.
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		ЗафиксироватьТранзакцию();
		Возврат Выборка.Ссылка;
...
Процедура АктивироватьВРМ(ВРМ) Экспорт
	...
	Блокировка = Новый БлокировкаДанных;
	ЭлементБлокировки = Блокировка.Добавить(ВРМ.Метаданные().ПолноеИмя());
	...
	Попытка
		...
		Если ВРМОбъект = Неопределено Тогда
			ВызватьИсключение НСтр(СтрШаблон("ru = 'Ошибка при активации ВРМ: ВРМ ""%1"" не найдено.'", ВРМ));
		КонецЕсли;
...
Процедура ВыгрузитьВРМ(ВРМ) Экспорт
	...
	Исключение
		// Может возникнуть исключение при конкурентной выгрузке ВРМ на перегруженных системах.
		// При массовой выгрузке агенты Тест-центра пытаются удалить ВРМ не только в рамках своей сессии ОС,
		// это сделано для случая, когда часть агентов по каким-либо причинам выходят из строя.
		Если ВРМ.ПолучитьОбъект() <> Неопределено Тогда
			ТекстОшибки = НСтр("ru = 'Ошибка при выгрузки ВРМ: '") + ТЦОбщий.ИнформациюОбОшибкеВСтроку(ИнформацияОбОшибке());
			ТЦОбщий.ЗаписатьВЖурнал(ТекстОшибки, НСтр("ru = 'ВРМ'"), Перечисления.ТЦСтатусСообщения.Предупреждение);
		КонецЕсли;
...
Процедура ЗагрузитьВРМ(СписокВРМ) Экспорт
	...
	РезультатЗапросаЗагрузки = Запрос.Выполнить();
	Если РезультатЗапросаЗагрузки.Пустой() Тогда
		Возврат;
	КонецЕсли;
	...
	Попытка
		...
		Если Метаданные.РежимУправленияБлокировкойДанных = Метаданные.СвойстваОбъектов.РежимУправленияБлокировкойДанныхПоУмолчанию.Управляемый Тогда
			Блокировка = Новый БлокировкаДанных;
			ЭлементБлокировки = Блокировка.Добавить("Справочник.ТЦВП");
			ЭлементБлокировки.ИсточникДанных = РезультатЗапросаЗагрузки;
			ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Ссылка", "ВРМ");
			Блокировка.Заблокировать();
		КонецЕсли;
		
		Выборка = РезультатЗапросаЗагрузки.Выбрать();
		Пока Выборка.Следующий() Цикл
			
			ВРМОбъект = Выборка.ВРМ.ПолучитьОбъект();
			ВРМОбъект.Роль = Выборка.Роль;
			ВРМОбъект.ПродолжатьРаботуМеждуИтерациями = Выборка.ПродолжатьРаботуМеждуИтерациями;
			ВРМОбъект.Состояние = Перечисления.ТЦСостояние.Загрузка;
...
Процедура УдалитьВРМ(ВРМ) Экспорт
	...
	Исключение
		...
		// это сделано для случая, когда часть агентов по каким-либо причинам выходят из строя.
		Если ВРМ.ПолучитьОбъект() <> Неопределено Тогда
			ТекстОшибки = НСтр("ru = 'Ошибка при удалении ВРМ: '") + ТЦОбщий.ИнформациюОбОшибкеВСтроку(ИнформацияОбОшибке());
			ТЦОбщий.ЗаписатьВЖурнал(ТекстОшибки, НСтр("ru = 'ВРМ'"), Перечисления.ТЦСтатусСообщения.Предупреждение);
		КонецЕсли;
...
Процедура ИзменитьИмяВРМ(ИмяВРМ, Номер, СсылкаВРМ) Экспорт
	...
	Блокировка = Новый БлокировкаДанных;
	ЭлементБлокировки = Блокировка.Добавить(СсылкаВРМ.Метаданные().ПолноеИмя());
	ЭлементБлокировки.УстановитьЗначение("Ссылка", СсылкаВРМ);
	...
	Попытка
		...
		ВРМОбъект = СсылкаВРМ.ПолучитьОбъект();
		Если ВРМОбъект = Неопределено Тогда
			ВызватьИсключение НСтр("ru = 'На момент изменения имени ВРМ """ + ИмяВРМ + """ уже было удалено.'");
		КонецЕсли;
		
		ВРМОбъект.ОбменДанными.Загрузка = Истина;
		ВРМОбъект.Наименование = ИмяВРМ;
		ВРМОбъект.Номер = Номер;
		ВРМОбъект.Записать();
		ЗафиксироватьТранзакцию();
	Исключение
		ОтменитьТранзакцию();
...
Процедура УстановитьРазделениеСеанса(Знач СтруктураРазделителей = Неопределено) Экспорт
	...
	Для Каждого Разделитель Из Разделители Цикл
		...
		ТипРазделителя = Разделитель.Тип;
		...
		Если СтруктураРазделителей.Свойство(ИмяРазделителя) Тогда
			ПараметрыСеанса[ПараметрСеансаЗначение] = ТипРазделителя.ПривестиЗначение(СтруктураРазделителей[ИмяРазделителя]);
...
//
Функция БСПРазделениеВключено() Экспорт
	
	Если Не БСПЭтоРазделеннаяКонфигурация() Тогда
		Возврат Ложь;
	КонецЕсли;
...
//
Функция БСПЭтоРазделеннаяКонфигурация() Экспорт
...
Функция БСПСоздатьПользователей(ПользователиИБ)
	...
	ИспользуетсяРазделениеВМоделиСервиса = Ложь;
	Если БСППодсистемаИспользуется("РаботаВМоделиСервиса") Тогда
		// Если внедрена подсистема "Работа в модели сервиса", то 
		// выключать разделение нужно с её помощью
		ИспользуетсяРазделениеВМоделиСервиса = БСПРазделениеВключено();
	КонецЕсли;
	...
	Для Каждого ПользовательИБ Из ПользователиИБ Цикл
		...
		Если Не ЗначениеЗаполнено(Пользователь) Тогда
			...
			Если ИспользуетсяРазделениеВМоделиСервиса Тогда
...
//
Процедура УстановитьТипКлиентскогоПриложения(ТЦТипПриложения) Экспорт
	
	Если Не ТЦТипПриложения = ПараметрыСеанса.ТЦТипКлиентскогоПриложения Тогда
		...
		Если ЗначениеЗаполнено(ЭлементУправлениеТестом) Тогда
			...
			НовыйОбъект.ТипПриложения = ТЦТипПриложения;
			НовыйОбъект.ОбменДанными.Загрузка = Истина;
			НовыйОбъект.Записать();
			ПараметрыСеанса.ТЦТипКлиентскогоПриложения = ТЦТипПриложения;
...
Функция ПолучитьИнтервалОпроса() Экспорт
	...
	Интервал = ТЦОбщий.ИнтервалОпросаУправленияТестированиемПоУмолчанию();
	Если Метаданные.Константы.Найти("ТЦИнтервалОпросаУправленияТестированием") <> Неопределено Тогда
