...
Функция ВыполнитьШагТеста() 
	...
	Иначе  
		Попытка               
			...
			СоздатьДокументКопированием(ТЦОбъект.Документы[ИндексТЧДокументы].ДокументПродажи);
...
Процедура СоздатьДокументКопированием(Ссылка) 
	...
	ОценкаПроизводительности.ЗакончитьЗамерВремени("Документ._ДемоРеализацияТоваров.СозданиеКопированием", НачалоЗамера);
КонецПроцедуры
...
Процедура ПометитьДокументыНаУдаление()
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |ГДЕ
	                      |	НЕ _ДемоРеализацияТоваров.ПометкаУдаления
	                      |	И ПОДСТРОКА(_ДемоРеализацияТоваров.Комментарий, 1, 3) = ""#ТЦ""");
