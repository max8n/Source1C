...
&НаКлиенте
Перем СозданоДокументов, ИндексТЧДокументы;
...
Функция ВыполнитьШагТеста() 
	...
	Если СозданоДокументов >= ТЦОбъект.КоличествоДокументов Тогда
		ТЦОбщий.ЗаписатьВЖурнал("СозданоДокументов >= ТЦОбъект.КоличествоДокументов", "НагрузочныйТестОтладка", ТЦСтатусСообщенияПредупреждение());
		ТЦЗавершитьВыполнение(ПредопределенноеЗначение("Перечисление.ТЦРезультатВыполнения.Успешно"), Ложь);
		Возврат Неопределено;  
	Иначе  
		Попытка               
			ИндексТЧДокументы = ?(ИндексТЧДокументы >= ТЦОбъект.Документы.Количество(),0,ИндексТЧДокументы);
			СоздатьДокументКопированием(ТЦОбъект.Документы[ИндексТЧДокументы].ДокументПоступления);
			СозданоДокументов = СозданоДокументов + 1;  
			ИндексТЧДокументы = ИндексТЧДокументы + 1;
		Исключение                                 
			ОписаниеОшибки = ОписаниеОшибки();
			ТЦОбщий.ЗаписатьВЖурнал(ОписаниеОшибки, "НагрузочныйТест", ТЦСтатусСообщенияПредупреждение());   
			ТЦЗавершитьВыполнение(ПредопределенноеЗначение("Перечисление.ТЦРезультатВыполнения.Ошибка"), Истина, ОписаниеОшибки);
		КонецПопытки;		
	КонецЕсли;
	
	ТЦРезультатВыполненияПродолжить("ВыполнитьШагТеста", ТЦОбъект.Пауза, Истина);
КонецФункции  

&НаСервереБезКонтекста
Процедура СоздатьДокументКопированием(Ссылка) 
	НачалоЗамера = ОценкаПроизводительности.НачатьЗамерВремени();
	ДокОбъект = Ссылка.Скопировать();
	ДокОбъект.Дата = ТекущаяДата();
	ДокОбъект.Комментарий = "#ТЦ";
	ДокОбъект.Записать(РежимЗаписиДокумента.Проведение);
	ОценкаПроизводительности.ЗакончитьЗамерВремени("Документ._ДемоПоступлениеТоваров.СозданиеКопированием", НачалоЗамера);
КонецПроцедуры
...
Функция ТЦВыполнить() Экспорт
	...
	Возврат ТЦРезультатВыполненияПродолжить("ВыполнитьШагТеста", ТЦОбъект.Пауза, Истина);
...
Процедура ПометитьДокументыНаУдаление()
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |ГДЕ
	                      |	НЕ _ДемоПоступлениеТоваров.ПометкаУдаления
	                      |	И ПОДСТРОКА(_ДемоПоступлениеТоваров.Комментарий, 1, 3) = ""#ТЦ""");
...
&НаКлиенте
// Номер ВРМ, уникальный в рамках данной роли сценария.
//
// Возвращаемое значение:
