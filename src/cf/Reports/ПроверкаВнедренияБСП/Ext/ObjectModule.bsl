		...
		Для Каждого ИмяПредопределенного Из ИменаПредопределенных Цикл
			...
			Если ПредопределенныеДанные.Найти(ИмяПредопределенного, "ИмяПредопределенныхДанных") = Неопределено Тогда
				...
				Кратко = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Для объекта ""%1"" отсутствуют код заполнения предопределенного элемента %2'"),  МетаданныеОбъекта.ПолноеИмя(), ИмяПредопределенного);
				Подробно = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Для объекта ""%1"" следует разместить код заполнения предопределенного элемента %2 
					|в процедуре %3 модуля менеджера объекта или в процедуре %3 общего модуля %4.'"),
					МетаданныеОбъекта.ПолноеИмя(), ИмяПредопределенного, "ПриНачальномЗаполненииЭлементов", "ОбновлениеИнформационнойБазыПереопределяемый");
