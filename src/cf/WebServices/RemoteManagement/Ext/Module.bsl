...
Функция GetTestManagmentList() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.ТипПриложения) Тогда
			ТЦТипКлиентскогоПриложения = ФабрикаXDTO.Создать(ТипКлиентскогоПриложенияТип);
			Индекс = Перечисления.ТЦТипКлиентскогоПриложения.Индекс(Выборка.ТипПриложения);
			ТЦТипКлиентскогоПриложения.ID = Метаданные.Перечисления.ТЦТипКлиентскогоПриложения.ЗначенияПеречисления[Индекс].Имя;
			ТЦТипКлиентскогоПриложения.Name = Выборка.ТипПриложенияПредставление;
		Иначе
			ТЦТипКлиентскогоПриложения = Неопределено;
		КонецЕсли;
		...
		Клиент.ClientAppType = ТЦТипКлиентскогоПриложения;
...
Функция SetTestManagementByID(ClientUUID, TypeID, StartDate, FinishDate, ScenarioUUID)
	...
	Если ЗначениеЗаполнено(TypeID) Тогда
		Попытка
			ТЦТипПриложения = Перечисления.ТЦТипКлиентскогоПриложения[TypeID];
		Исключение
			Возврат "Неверное значение параметра ""TypeID"" (" + TypeID + ")";
		КонецПопытки;
	Иначе
		ТЦТипПриложения = Перечисления.ТЦТипКлиентскогоПриложения.ПустаяСсылка();
	КонецЕсли;
	Если ТЦТипПриложения = Перечисления.ТЦТипКлиентскогоПриложения.УправлениеТестом Тогда
	...
	СпрОбъект.ТипПриложения = ТЦТипПриложения;
